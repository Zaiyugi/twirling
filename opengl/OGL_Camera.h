...
public:
  ...
  OGL_Camera(Vector3d P, Vector3d A, Vector3d U);
  ...
  Vector3d view(const double x, const double y, int W, int H);
  
  // function that handles mouse events
  int HandleMouseEvent(int button, int state, int x, int y);
  
  // function that handles mouse movements
  void HandleMouseMotion(int x, int y);
  
  const OGL_Camera& operator=(const OGL_Camera& cam);
