...
void OGL_Camera::ComputeCoords(const Vector3d &P, const Vector3d &A, const Vector3d &U){
  Vector3d zaxis = P - A;
 ...
 * U is the up vector 
 */
OGL_Camera::OGL_Camera(Vector3d P, Vector3d A, Vector3d U) {
  ComputeCoords(P, A, U);
...
void OGL_Camera::SetPos(Vector3d P) {
  ComputeCoords(P, Aim, Up);
}

void OGL_Camera::SetAim(Vector3d A) {
  ComputeCoords(Pos, A, Up);
}

void OGL_Camera::SetUp(Vector3d U) {
 ...
 */
void OGL_Camera::SetClippingPlanes(float Near, float Far) {
  NearPlane = Near;
  FarPlane = Far;
}
 ...
 */
void OGL_Camera::SetFOV(float ViewAngle) {
  Fov = ViewAngle;
}
 ...
 */
void OGL_Camera::SetCenterOfFocus(Vector3d NewAim) {
 ...
 */
void OGL_Camera::PerspectiveDisplay(int W, int H) {
...
Vector3d OGL_Camera::view(const double x, const double y, int W, int H)
{
   double aspect_ratio = static_cast<double>(W) / static_cast<double>(H);
   double htanfov = tan(Fov*0.5*M_PI/180.0);
   double vtanfov = htanfov/aspect_ratio;

   double xx = (2.0 * x - 1.0) * htanfov;
   double yy = (2.0 * y - 1.0) * vtanfov;
   Vector3d dir = (Aim - Pos).normalize();
   return (Up * yy + (dir % Up) * xx + dir).normalize();
}
 ...
 */
int OGL_Camera::HandleMouseEvent(int button, int state, int x, int y) {
 ...
 */
void OGL_Camera::HandleMouseMotion(int x, int y) {
...
// assignment operator
const OGL_Camera& OGL_Camera::operator=(const OGL_Camera& Cam) {
  Aim = Cam.Aim;
  Pos = Cam.Pos;
  Up = Cam.Up;
